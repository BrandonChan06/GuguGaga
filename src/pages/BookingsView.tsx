      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-[20px] sm:text-[22px] font-extrabold text-slate-900 tracking-tight">
            Bookings, Vouchers & Live Pricing
          </h1>
          <p className="text-[11px] sm:text-[12px] text-slate-500 mt-0.5">
            Store confirmation codes, track price trends & toggle Booked vs Pending status
          </p>
        </div>
        <div className="flex items-center gap-2.5 sm:gap-3 flex-wrap">
          {/* Status filter toggle */}
          <div className="flex items-center bg-slate-100 p-1 rounded-xl text-[11px] sm:text-[12px] font-bold">
            {(['all', 'booked', 'pending'] as const).map(s => (
              <button
                key={s}
                onClick={() => setFilterStatus(s)}
                className={`px-2.5 sm:px-3 py-1.5 rounded-lg capitalize transition-all cursor-pointer ${
                  filterStatus === s ? 'bg-white text-blue-600 shadow-xs' : 'text-slate-500 hover:text-slate-800'
                }`}
              >
                {s === 'all' ? 'All' : s === 'booked' ? 'Booked ✅' : 'Pending ⏳'}
              </button>
            ))}
          </div>
      ...
      {displayedBookings.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {displayedBookings.map(b => (
            <div key={b.id} className="bg-white p-4 sm:p-5 rounded-3xl border border-slate-100 shadow-sm space-y-3.5 hover:shadow-md transition-all">
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3">
                  <div className="w-10 sm:w-11 h-10 sm:h-11 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-center text-xl sm:text-2xl flex-shrink-0">
                    {b.emoji}
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="text-[13px] font-extrabold text-slate-900 leading-tight">{b.title}</span>
                    </div>
              ...
              <div className="grid grid-cols-2 gap-2 p-3 rounded-2xl bg-slate-50 border border-slate-100 text-[11px]">
                <div>
                  ...
                  <span className="font-mono font-bold text-slate-800 text-[11px] sm:text-[12px]">{b.confirmationCode}</span>
                </div>
        ...
        <div className="p-8 sm:p-12 text-center rounded-3xl bg-white border border-dashed border-slate-200 text-slate-400 space-y-3">
