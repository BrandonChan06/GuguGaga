...
export default function DashboardView({
  ...
  return (
    <div className="p-3.5 sm:p-6 md:p-8 max-w-[1200px] mx-auto space-y-5 sm:space-y-7 animate-fadeIn select-none">
      ...
      <div className="relative rounded-3xl overflow-hidden bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 text-white shadow-xl shadow-slate-900/10 border border-slate-800 p-4 sm:p-6 md:p-8">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-5 sm:gap-6">
          <div className="space-y-2.5 max-w-[620px]">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 backdrop-blur-md border border-white/15 text-[10px] sm:text-[11px] font-bold text-blue-200">
            ...
            <h1 className="text-xl sm:text-2xl md:text-3xl font-black tracking-tight text-white flex items-center gap-2 sm:gap-2.5 flex-wrap">
              <span>{currentTrip.destination}, {currentTrip.country}</span>
              <span className="text-xl sm:text-2xl">{currentTrip.flag}</span>
            </h1>
            ...
            <div className="pt-1.5 sm:pt-2 max-w-[420px] space-y-1.5">
              ...
              <div className="h-2 bg-white/15 rounded-full overflow-hidden backdrop-blur-xs">
                <div className="h-full bg-gradient-to-r from-blue-500 to-emerald-400 rounded-full transition-all duration-500" style={{ width: `${currentTrip.progress}%` }} />
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2.5 sm:gap-3">
            <button
              ...
              className="flex-1 sm:flex-initial px-4 sm:px-5 py-2.5 sm:py-3 rounded-2xl bg-blue-600 hover:bg-blue-500 text-white text-[12px] sm:text-[13px] font-bold shadow-lg shadow-blue-600/30 transition-all flex items-center justify-center gap-2 group cursor-pointer"
            >
            ...
            <button
              onClick={onOpenAI}
              className="flex-1 sm:flex-initial px-3.5 sm:px-4 py-2.5 sm:py-3 rounded-2xl bg-white/10 hover:bg-white/15 text-white text-[12px] sm:text-[13px] font-bold border border-white/20 backdrop-blur-md transition-all flex items-center justify-center gap-2 cursor-pointer"
            >
      ...
      {pendingBookings.length > 0 && (
        <div className="p-3.5 sm:p-5 rounded-2xl bg-amber-50/90 border border-amber-200/80 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-xs">
          <div className="flex items-start sm:items-center gap-3">
            <div className="w-8 sm:w-9 h-8 sm:h-9 rounded-xl bg-amber-100 text-amber-700 flex items-center justify-center text-base sm:text-lg flex-shrink-0 font-bold">
              ⏳
            </div>
            <div>
              <div className="text-[12px] sm:text-[13px] font-bold text-amber-950">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 sm:gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-3xl border border-slate-200/80 p-4 sm:p-6 shadow-sm hover:shadow-md transition-shadow space-y-4 flex flex-col justify-between">
          {nextActivity ? (
            <>
              <div className="space-y-3">
                ...
                <div className="flex items-start gap-3 sm:gap-4 pt-1">
                  <div className="w-11 sm:w-13 h-11 sm:h-13 rounded-2xl bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-100 flex items-center justify-center text-2xl sm:text-3xl flex-shrink-0 shadow-xs">
                    {nextActivity.emoji}
                  </div>
                  <div className="space-y-1 min-w-0 flex-1">
                    <h3 className="text-[16px] sm:text-[18px] font-extrabold text-slate-900 tracking-tight leading-tight">
                      {nextActivity.name}
                    </h3>
                    <div className="flex flex-wrap items-center gap-y-1 gap-x-2.5 text-[11px] sm:text-[12px] text-slate-500 font-medium">
              ...
              <div className="pt-3 border-t border-slate-100 flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
                  ...
                  <button
                    onClick={() => onNavigate('itinerary')}
                    className="px-3.5 sm:px-4 py-1.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-[11px] font-bold shadow-xs transition-colors flex items-center gap-1 cursor-pointer"
                  >
            ...
            <div className="flex flex-col items-center justify-center text-center p-6 sm:p-8 space-y-3.5 my-auto">
              <div className="w-12 sm:w-14 h-12 sm:h-14 rounded-2xl bg-blue-50 border border-blue-100 text-blue-600 flex items-center justify-center text-2xl sm:text-3xl font-bold shadow-xs">
                📍
              </div>
              <div className="space-y-1">
                <h3 className="text-[15px] sm:text-[17px] font-bold text-slate-900">
                  No activities scheduled for {currentTrip.destination} yet
                </h3>
                <p className="text-[11px] sm:text-[12px] text-slate-500 max-w-[380px] leading-relaxed">
                  Start building your personalized itinerary, add landmark stops, or ask the AI Concierge for recommendations.
                </p>
              </div>
              <button
                onClick={() => onNavigate('itinerary')}
                className="px-4 sm:px-5 py-2 sm:py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-[11px] sm:text-[12px] font-bold rounded-xl shadow-xs transition-colors cursor-pointer"
              >
        ...
        <div className="bg-white rounded-3xl border border-slate-200/80 p-4 sm:p-6 shadow-sm flex flex-col justify-between space-y-4">
          <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-5">
        ...
        <div
          ...
          className="bg-white p-4 sm:p-5 rounded-3xl border border-slate-200/80 shadow-sm hover:border-blue-300 hover:shadow-md transition-all cursor-pointer group space-y-3"
        >
          <div className="flex items-center justify-between text-slate-400">
            <span className="text-[10px] sm:text-[11px] font-bold uppercase tracking-wider text-slate-500">Budget & Spending</span>
          ...
          <div>
            <div className="text-[20px] sm:text-[24px] font-black text-slate-900 tracking-tight">
        ...
        <div
          onClick={() => onNavigate('bookings')}
          className="bg-white p-4 sm:p-5 rounded-3xl border border-slate-200/80 shadow-sm hover:border-emerald-300 hover:shadow-md transition-all cursor-pointer group space-y-3"
        >
          <div className="flex items-center justify-between text-slate-400">
            <span className="text-[10px] sm:text-[11px] font-bold uppercase tracking-wider text-slate-500">Bookings Status</span>
        ...
        <div
          onClick={() => onNavigate('itinerary')}
          className="bg-white p-4 sm:p-5 rounded-3xl border border-slate-200/80 shadow-sm hover:border-indigo-300 hover:shadow-md transition-all cursor-pointer group space-y-3"
        >
          <div className="flex items-center justify-between text-slate-400">
            <span className="text-[10px] sm:text-[11px] font-bold uppercase tracking-wider text-slate-500">Itinerary Schedule</span>
      ...
      <div className="bg-white rounded-3xl border border-slate-200/80 p-4 sm:p-6 shadow-sm space-y-4 sm:space-y-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-2 border-b border-slate-100">
          <div>
            <h2 className="text-[15px] sm:text-[16px] font-extrabold text-slate-900 tracking-tight">
        ...
        {day1Stops.length > 0 ? (
          <div className="space-y-2.5 sm:space-y-3">
            {day1Stops.map((stop, idx) => (
              <div
                key={stop.id}
                className="p-3 sm:p-3.5 rounded-2xl bg-slate-50/80 hover:bg-blue-50/40 border border-slate-100 hover:border-blue-200 transition-all flex items-center justify-between gap-3"
              >
                <div className="flex items-center gap-2.5 sm:gap-3.5 min-w-0">
                  {/* Time badge */}
                  <div className="w-14 sm:w-16 flex-shrink-0 text-center">
                    <span className="text-[11px] sm:text-[12px] font-black text-slate-800 block leading-tight">{stop.time}</span>
                    <span className="text-[9px] sm:text-[10px] text-slate-400 font-medium">to {stop.endTime}</span>
                  </div>

                  {/* Emoji & Name */}
                  <div className="w-8 sm:w-9 h-8 sm:h-9 rounded-xl bg-white border border-slate-200/80 flex items-center justify-center text-base sm:text-lg flex-shrink-0 shadow-xs">
                    {stop.emoji}
                  </div>

                  <div className="min-w-0">
                    <div className="text-[12px] sm:text-[13px] font-bold text-slate-900 truncate">
                      {stop.name}
                    </div>
                ...
                <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
                  ...
                  <span className="text-[11px] sm:text-[12px] font-extrabold text-slate-800">
                    {stop.cost || 'Free'}
                  </span>
                </div>
              </div>
          ...
          <div className="p-6 sm:p-8 text-center rounded-2xl bg-slate-50 border border-dashed border-slate-200 text-slate-400 space-y-2">
